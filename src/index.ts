...
Pass the uuids returned by search_code. EFFICIENCY: trust a high-fit search result — do NOT get_file/get_method to "double-check" before referencing, and do NOT get_assembled_code to verify after; the reference IS the proven code. References are VERBATIM by design: there is no rename step. To adapt the reused code (different names, fields, behaviour), just edit it afterwards with the normal editing tools — each edit forks only the touched code into your project automatically (copy-on-write), leaving the canonical unit and every other project referencing it untouched. ROUTES ARE AUTOMATIC: referencing a controller auto-carries the routes that point at it (rewired to your project) — you do NOT need to pass them in 'routes' or rebuild them with create_route afterwards. The source project is never modified. Returns the new file/route uuids + reference counts; then just run_migration for any carried migrations. So the whole reuse is ONE call: search_code → reuse_code(files) → run_migration.`,
  ...
  {
    name: 'see_divergences',
    description: `See how PEERS changed the canonical code your project references — the reasoned divergences other projects made from units you reference ("someone on this code changed it, here's why"). Use it when adapting reused code: before you re-implement a change to a referenced unit, check whether another project already diverged the same way and why — you may want the same edit, or it may warn you off. Returns { canonical_uuid, resource_type, reason, diverged_at } per divergence; EXPLAINED divergences only (an unexplained fork is noise), and never your own. Read-only, no input.`,
    inputSchema: {
      type: 'object',
      properties: {},
    },
  },
  {
    ...
    description: `Explain a divergence you just made — attach the WHY to a referenced unit your project forked. When you edit code that was referenced (not authored) here, copy-on-write forks a private copy; call this right after so other projects on the same canonical unit can see why you changed it (and, later, adopt it). Pass fork_uuid = the uuid of the referenced unit you just edited, and a one-line reason. Only the project that made the fork can explain it, and only a real divergence of a referenced unit qualifies (editing your own code records nothing to explain).`,
    inputSchema: {
      type: 'object',
      properties: {
        fork_uuid: {
          type: 'string',
          description: 'UUID of the referenced unit you just edited (its local uuid — the same one you passed to the edit tool).',
        },
        reason: {
          type: 'string',
          description: 'One line: what you changed and why (max 500 chars). This is the signal other referencers see.',
        },
      },
      required: ['fork_uuid', 'reason'],
    },
  },
  {
...
const LAZY_GROUPS: Record<string, string[]> = {
  ...
  // The user's relationship with the shared library — submitting their OWN code and checking how
...
async function handleCallTool(request: any) {
  ...
  try {
    switch (name) {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                message: enabled.length > 0
                  ...
                  : 'No known groups matched. Valid groups: editing, frontend, analysis, capabilities, settings, contribute.',
              }, null, 2),
            },
          ],
        };
      ...
      case 'see_divergences': {
        const result = await stellify.seeDivergences();
        const divergences = result.data || [];
        return {
          content: [
            {
              type: 'text',
              text: JSON.stringify({
                ...
                message: result.message,
                divergences,
                next_step: divergences.length
                  ? 'A peer changed a unit you reference — consider whether you want the same edit. Editing it forks your own copy (copy-on-write); explain_divergence yours to pass the reasoning on.'
                  : 'No explained peer divergences on the units you reference.',
              }, null, 2),
            },
          ],
        };
      ...
      case 'explain_divergence': {
        const result = await stellify.explainDivergence(args as any);
        const data = result.data || result;
        return {
          content: [
            {
